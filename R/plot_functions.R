...
#' \code{\link{est_brc}}) is ploted on a single frame by index.  This can be
...
plot_lof <- function(brc, min_lof = 1) {
  # brc is a data frame containging Bioric Response Curves generated by
  # function est_brc.
...
plot_brc <- function(sp, brc, ref_grad) {
  ...
  for (species in 1:ncol(sp)) {
    with(brc[species, ], {
      ...
      plot(sp[, species] ~ ref_grad, main = names(sp)[species],
...
#'
#' @param title the name taxanomic group being plotted (e.g., "fish").
...
plot_iec_cor <- function (iec, env_grad, title) {
  plotstats <-lm(iec$IEC ~ env_grad[, 1])
