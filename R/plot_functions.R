...
#'
#' The optional arguments, \code{ref_bad} and \code{ref_good}, provide a way to
...
#'
#' @param ref_bad numeric scalar worst condition of original environmental
#'   gradient (optional).
#' @param ref_good numeric scalar best condition of original environmental
#'   gradient (optional).
...
plot_brc <- function(sp, brc, ref_grad, ref_bad = NULL, ref_good = NULL,
  ...
  for (species in 1:ncol(sp)) {
    with(brc[species, ], {
      ...
      mtext(ref_bad, side = 1, line = 2, adj = 0.03, cex = 0.8)
      mtext(ref_good, side = 1, line = 2, adj = 0.97, cex = 0.8)
...
brc_pdf <- function(out_pdf, sp, brc, ref_grad, min_lof = 1,
                    ref_bad = NULL, ref_good = NULL, ylab = "Response",
  ...
  if (file.exists(out_pdf)) {
    ...
    tryCatch({
      ...
      plot_brc(sp, brc, ref_grad, ref_bad, ref_good, ylab, sub_xlab, sub_main)
