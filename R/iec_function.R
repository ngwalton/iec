...
# BR functions can be calculated using function "mk_brc".
#
...
# where present/absence data are available, and method "calcLSq" is used if
# probability data are available.
#
# Note: Future versions of "iec_score" may be written so that only species in
...
#'
#' \code{iec_score} estimates the Index of Ecological Condition (IEC) for each
...
#' observed taxa.  Need to indicate when you'd use "calcLSq" vs "calcLik".
#' \code{n_reps} determines the number of times the optimization function
#' (\code{\link[stats]{nlminb}}) is run with different starting values.  The
#' column names in \code{sp} and the first column of \code{brc} must contain the
#' same taxa and order. \code{n_reps} can be set to any positive integer, but
#' the authors recommend against using values of less than 10 except for testing
#' purposes.  Larger values will take longer to process.  The default value is
#' 30.
#'
#' @param brc BRC data frame generated by \code{\link{mk_brc}}
#' @param method one of "calcLSq" or "calcLik" (default).
#' @param n_reps numeric scalar setting the number of random starts used in
#'   optimization (defaults to 30).
...
#' @seealso \code{\link{mk_brc}} to generate biotic response curves.
